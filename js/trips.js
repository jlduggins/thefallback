...
const Trips = {
  ...
  openJourneyDetail(journeyId) {
    ...
    panelContent.innerHTML = `
      ...
      <div class="journey-detail-scroll" style="flex:1;overflow-y:auto;min-height:0;-webkit-overflow-scrolling:touch">
  ...
  async refreshAllRoutes(journeyId){
    const j=State.getJourney(journeyId);if(!j?.legs?.length)return;
    // Preserve scroll position across the re-render at the end so the user
    // doesn't get jumped to the top of the list after clicking a badge.
    const scrollEl=document.querySelector('.journey-detail-scroll');
    const savedScroll=scrollEl?scrollEl.scrollTop:0;
    const legs=[...j.legs];
    for(let i=0;i<legs.length;i++){
      ...
      let fLat,fLng;
      if(i===0){
        fLat=leg.fromLat;fLng=leg.fromLng;
        if(!fLat&&State.userLat){fLat=State.userLat;fLng=State.userLng;leg.fromLat=fLat;leg.fromLng=fLng;leg.fromName='Current Location';}
      }else{
        // For legs past index 0, the "from" is always the previous leg's destination.
        // Persist these onto the leg so the post-render stale check clears.
        fLat=legs[i-1].destLat;fLng=legs[i-1].destLng;
        leg.fromLat=fLat;leg.fromLng=fLng;
        leg.fromName=legs[i-1].destName;
        leg.fromId=legs[i-1].destId;
      }
      if(fLat&&leg.destLat){
        try{
          const r=await this.getRoute(fLat,fLng,leg.destLat,leg.destLng);
          if(r){
            leg.distance=Math.round(r.distance);
            leg.duration=Math.round(r.duration);
            leg.fuelCost=Math.round(this.calcFuelCost(r.distance));
            if(r.geometry)leg.routeGeometry=JSON.stringify(r.geometry);
          }
        }catch(e){}
      }
      legs[i]=leg;
    ...
    await Firebase.saveJourney({...j,legs});
    this.openJourneyDetail(journeyId);
    // Restore scroll: rAF alone can fire before the new content has layout, so
    // try across two frames + a fallback timeout to cover late layout passes
    // (Sortable init, map invalidateSize, etc.).
    const restore=()=>{const el=document.querySelector('.journey-detail-scroll');if(el)el.scrollTop=savedScroll;};
    requestAnimationFrame(()=>{restore();requestAnimationFrame(restore);setTimeout(restore,80);});
  },
